...
public class ProductController {
    ...
    * @return
    */
    @RequestMapping(value="/productDL/{id}",method=RequestMethod.DELETE)
     ...
     * @return
     */
    @RequestMapping(value = "/product/batchDelete",method = RequestMethod.GET)
    @ResponseBody
    public AjaxResult batchDelete(String idsStr){
        try {
            String[] ids = idsStr.split(",");
            List<String> list = Arrays.asList(ids);
            productService.batchDeleteByIds(list);
            return AjaxResult.me();
        } catch (Exception e) {
            e.printStackTrace();
            return AjaxResult.me().setMessage("删除对象失败！"+e.getMessage());
