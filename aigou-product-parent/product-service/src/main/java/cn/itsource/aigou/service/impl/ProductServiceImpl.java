...
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product> implements IProductService {
    ...
    public PageList<Product> getByQuery(ProductQuery query) {
        ...
        IPage<Product> ipage = baseMapper.getSelectByQuery(page,query);
    ...
    public void onSale(List<Long> ids) {
        ...
        List<ProductDoc> productDocs = TypeCasting(products);
        //静态化该商品页面
    ...
    @Override
    @Transactional
    public void offSale(List<Long> idsStr) {
        //修改商品状态和时间并保存到数据库
        baseMapper.offSale(idsStr,new Date().getTime());
        //查询数据库
        elasticSearchClient.deleteBatchByIds(idsStr);
    }
     ...
     * @return
     */
    public List<ProductDoc> TypeCasting(List<Product> products){
        List<ProductDoc> productDocs = new ArrayList<>();
        for (Product product : products) {
            //将Product转ProductDoc
            ProductDoc productDoc = TypeCastings(product);
     ...
     * @return
     */
    public ProductDoc TypeCastings(Product product){
    ...
    private void staticProductDetail(Product product) {
        //模板路径
        String templatePath = "E:\\IDEA\\workspace\\aigou-parent\\aigou-product-parent\\product-service\\src\\main\\resources\\template\\productDetail\\product-detail.vm";
        //生成的文件的路径
        String targetPath = "E:\\IDEA\\workspace\\aigou-web-parent\\ecommerce\\"+product.getId()+".html";
        //数据
